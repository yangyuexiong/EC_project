...
crm_user_and_role = db.Table(
    'crm_user_and_role',
    db.Column('cms_admin_id', db.Integer, db.ForeignKey('ec_crm_admin.id'), primary_key=True, comment='用户id'),
    db.Column('cms_role_id', db.Integer, db.ForeignKey('ec_crm_role.id'), primary_key=True, comment='角色id'),
    comment='用户_角色_中间表'
)

crm_permission_and_role = db.Table(
    'cms_permission_and_role',
    db.Column('cms_permission_id', db.Integer, db.ForeignKey('ec_crm_permission.id'), primary_key=True, comment='权限id'),
    db.Column('cms_role_id', db.Integer, db.ForeignKey('ec_crm_role.id'), primary_key=True, comment='角色id'),
    comment='权限_角色_中间表'
)
